...
#define HYDRA_STR(x) #x
#define HYDRA_TONAME(aname)                                                    \
  eosio::name { HYDRA_STR(aname) }

#define HYDRA_POPULATE_TABLE(r, dummy, field)                                  \
  case HYDRA_TONAME(BOOST_PP_SEQ_ELEM(0, field)).value: {                      \
    hydra_insert_row<HYDRA_TONAME(BOOST_PP_SEQ_ELEM(0, field)),                \
  ...
  ACTION hydraload(const std::vector<hydraload_payload> payload) {             \
    ...
    for (auto row : payload) {                                                 \
      switch (row.table_name.value) {                                          \
        BOOST_PP_SEQ_FOR_EACH(HYDRA_POPULATE_TABLE, DUMMY_MACRO, TABLES)       \
      default:                                                                 \
